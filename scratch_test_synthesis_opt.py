...
print("=" * 80)
print("  GEMINI 3.7-FLASH OPTIMIZED SYNTHESIS & BENCHMARK TEST")
print("=" * 80)
...
print(f"2. Context Assembly Time: {t_context:.3f}s (Compact Context Size: {len(compact_grounding)} chars vs 44k raw)")

# Step 3: Call Gemini with gemini-3.7-flash and robust retries
t0 = time.time()

def call_gemini_optimized(api_key: str, condition: str, setting: str, grounding_text: str):
  ...
  "egypt_practice_and_pharmacology": {{
    ...
    for model in models_to_try:
        ...
        for attempt in range(1, 3):
            try:
                print(f"  -> Attempting Gemini call with '{model}' (Attempt {attempt})...")
                ...
                if attempt < 2:
                    ...
                    continue
                break
    raise RuntimeError("All Gemini synthesis attempts failed across models.")

res_json, used_model = call_gemini_optimized(api_key, condition, setting, compact_grounding)
t_gemini = time.time() - t0
t_total = time.time() - t_start_total

print(f"3. Gemini Request Time: {t_gemini:.2f}s (Used Model: {used_model})")
print(f"4. Total End-to-End Request Time: {t_total:.2f}s")
print(f"5. Final Response Status: SUCCESS (Generated {len(json.dumps(res_json))} bytes of structured JSON)")

print("\n=== VERIFICATION OF RESULT CONTENTS ===")
print(f"• Condition Name: {res_json.get('condition_name')}")
print(f"• Guidelines Listed: {len(res_json.get('guidelines', {}).get('authoritative_guidelines', []))}")
for g in res_json.get('guidelines', {}).get('authoritative_guidelines', [])[:2]:
    print(f"  - {g.get('organization')}: {g.get('guideline_title')}")
print(f"• Management Steps: {len(res_json.get('guidelines', {}).get('stepped_management_protocol', []))}")
print(f"• Egyptian Brands: {len(res_json.get('egypt_practice_and_pharmacology', {}).get('track_d_and_e_medication_landscape', []))}")
for b in res_json.get('egypt_practice_and_pharmacology', {}).get('track_d_and_e_medication_landscape', [])[:3]:
    print(f"  - {b.get('active_ingredient')} -> {b.get('famous_egyptian_brands')} ({b.get('reported_price_range_egp')})")
