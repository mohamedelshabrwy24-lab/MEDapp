...
print("=" * 80)
print("  MEDREF REAL SYNTHESIS BENCHMARK: DKA (EMERGENCY)")
print("=" * 80)
...
# Step 3: Synthesis with Cascade: gemini-3.7-flash (Preferred Production) -> gemini-3.5-flash-lite / gemini-3.1-flash-lite
t0 = time.time()

def call_gemini_production_cascade(api_key: str, condition: str, setting: str, grounding_text: str):
    # Production Model Preference Cascade
    models = [
        "gemini-3.7-flash",
        "gemini-3.6-flash",
...
res_json, active_model = call_gemini_production_cascade(api_key, condition, setting, compact_grounding)
...
print(f"• Guidelines Listed ({len(res_json.get('guidelines', {}).get('authoritative_guidelines', []))} guidelines):")
