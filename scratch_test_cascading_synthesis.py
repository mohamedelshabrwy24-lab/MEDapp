...
print("=" * 80)
print("  E2E REAL TEST: DKA EMERGENCY SYNTHESIS")
print("=" * 80)
...
# Step 3: Cascading Synthesis
t0 = time.time()

def call_gemini_cascading(api_key: str, condition: str, setting: str, grounding_text: str):
...
{{
  ...
  "egypt_practice_and_pharmacology": {{
    ...
    for model in models:
        ...
        # 15s timeout per model with 1 quick retry on 503/429
...
res_json, active_model = call_gemini_cascading(api_key, condition, setting, compact_grounding)
t_gemini = time.time() - t0
t_total = time.time() - t_start_total

print(f"3. Gemini Request Time       : {t_gemini:.2f}s (Successfully served by: {active_model})")
...
for step in res_json.get('guidelines', {}).get('stepped_management_protocol', [])[:3]:
    ...
    print(f"    Details: {step.get('clinical_details')[:90]}...")
...
print(f"• Verified Scientific Evidence: {res_json.get('egypt_practice_and_pharmacology', {}).get('track_b_scientific_and_epidemiological_evidence', {}).get('local_epidemiology_and_cohorts')[:100]}...")
print("=" * 80)
